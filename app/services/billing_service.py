...
def pay_bill_logic(subscriber_no, month):
    ...
    if bill:
        ...
        if remaining > 0:
            ...
            db.session.commit()
            return {"message": "Remaining bill amount paid", "new_total": bill.total}, 200
        else:
            return {"message": "Already fully paid", "total": bill.total}, 200

    new_bill = Bill(subscriber_no=subscriber_no, month=month, total=total, paid_at=now)
    db.session.add(new_bill)
    db.session.commit()

    return {"message": "Bill paid successfully", "total": total}, 200
